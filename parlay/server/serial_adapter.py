...
class SerialClientAdapter(Adapter, LineReceiver):
    """
    This class defines a Parlay client adapter for a serial port.

    This class would be used when the Parlay broker is running on some device
    on the other side of a serial port.

    **Example Usage**::

        client_adapter = SerialClientAdapter('/dev/ttyUSB0', baudrate=57600)
        my_item = MyItem('MyItemID', 'MyItemName', adapter=client_adapter)
    ...
    def __init__(self, port, baudrate=115200, delimiter='\n'):
        Adapter.__init__(self)
        self.delimiter = str(delimiter).decode("string_escape")
        self.transport = SerialPort(self, port, self.reactor, baudrate=baudrate)
    ...
    def lineReceived(self, line):
        ...
        # Special handling for the discovery request message
        if msg['TOPICS'].get('type', None) == 'get_protocol_discovery':
            resp = {
                'TOPICS': {
                    'type': 'get_protocol_discovery_response'
                },
                'CONTENTS': {
                    'discovery': [x.get_discovery() for x in self._items]
                }
            }
            return self.send_message_as_json(resp)

    ...
    def __init__(cls, name, bases, dct):
        ...
        if not hasattr(cls, 'message_registry'):
            ...
            if message_type_name in cls.message_registry:
                raise InvalidMessageTypeDeclaration(message_type_name +
                                                    " has already been declared. Please choose a different type name")
...
class BaseMessage(object):
    ...
    def to_message(self):
        ...
        if 'topics' not in self.msg:
            self.msg['topic'] = {}
        if 'contents' not in self.msg:
            self.msg['contents'] = {}
